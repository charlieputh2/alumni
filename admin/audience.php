					...
					<div class="card-body">
						...
						<table class="table table-bordered table-condensed table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$registering = $conn->query("SELECT a.*, e.title as event_title, e.schedule from audience a INNER JOIN events e ON e.id = a.event_id ORDER BY a.id DESC");
								...
								<tr>
									...
									<td class="">
										 <p>Event: <b><?php echo htmlspecialchars(ucwords($row['event_title'] ?? '')) ?></b></p>
										 <p><small>Schedule: <b><?php echo !empty($row['schedule']) ? date("M d, Y h:i A",strtotime($row['schedule'])) : 'N/A' ?></b></small></p>
									</td>
									<td class="">
										 <p>Name: <b><?php echo htmlspecialchars(ucwords($row['name'] ?? '')) ?></b></p>
										 <p><small>Email: <b><?php echo htmlspecialchars($row['email'] ?? '') ?></b></small></p>
										 <p><small>Contact: <b><?php echo htmlspecialchars($row['contact'] ?? '') ?></b></small></p>
									</td>
